...
export async function POST(request: NextRequest) {
  try { const data = Schema.parse(await request.json()); const run = await prisma.evaluationRun.create({ data: { ...data, metrics: data.metrics ? JSON.stringify(data.metrics) : null } }); return NextResponse.json({ success: true, data: run }, { status: 201 }) }
