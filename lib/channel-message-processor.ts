...
export async function processIncomingChannelMessage(input: { botId: string; channel: "whatsapp" | "instagram"; externalThreadId: string; externalMessageId: string; text: string; userName?: string; userPhone?: string }) {
  ...
  const context: OrchestratorContext = {
    ...
    botConfig: {
      ...
      role: settings.role, objective: settings.objective, rules: settings.rules, language: settings.language,
      tone: settings.tone, responseLength: settings.responseLength, fallbackMessage: settings.fallbackMessage,
  ...
  await prisma.conversation.update({ where: { id: conversation.id }, data: { lastMessageAt: new Date(), userIntent: result.decision.intent.intent, topicsDiscussed: stringifyJSON(Array.from(new Set([...(parseJSON<string[]>(conversation.topicsDiscussed) || []), ...result.decision.topics]))) } });
  return { duplicate: false as const, handoff: false as const, conversationId: conversation.id, assistantMessageId: assistantMessage.id, response: result.response };
}
