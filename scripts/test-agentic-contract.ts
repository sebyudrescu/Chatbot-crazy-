...
assert.match(orchestratorSource, /const hasPriorUserTurn = context\.conversationHistory\.some/)
assert.match(orchestratorSource, /const discoveryNeedsClarification = shouldClarifyProductDiscoveryPresentation/)
assert.match(orchestratorSource, /message:\s*context\.query,\s*hasPriorUserTurn,\s*query:\s*currentCommerceQuery,\s*semanticSearch:\s*latestProductSearchSignal/)
assert.match(orchestratorSource, /finalText = productDiscoveryClarification\(currentCommerceQuery\.category\)/)
assert.match(orchestratorSource, /if \(!discoveryNeedsClarification && productCards\.length === 0 && searchedProducts\.length > 0\)/)
assert.match(orchestratorSource, /if \(!discoveryNeedsClarification && actions\.forceProductCards && productCards\.length === 0\)/)
