...
assert.match(orchestratorSource, /const productDiscoveryAttempted = toolTrace\.some/)
assert.match(orchestratorSource, /trace\.name === ["']search_products["'] \|\| trace\.name === ["']present_products["']/)
assert.match(orchestratorSource, /const genericDiscoveryNeedsClarification = needsProductDiscoveryClarification/)
assert.match(orchestratorSource, /\) && productDiscoveryAttempted/)
assert.match(orchestratorSource, /finalText = productDiscoveryClarification\(currentCommerceQuery\.category\)/)
assert.match(orchestratorSource, /if \(!genericDiscoveryNeedsClarification && productCards\.length === 0 && searchedProducts\.length > 0\)/)
assert.match(orchestratorSource, /if \(!genericDiscoveryNeedsClarification && actions\.forceProductCards && productCards\.length === 0\)/)
