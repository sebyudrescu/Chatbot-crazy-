...
function latestActiveProductIds(
  ...
  for (const message of [...messages].reverse()) {
    ...
    const sourceMetadata = (
      parseJSON(message.sourcesUsed) as {
        metadata?: { activeProductIds?: unknown };
      } | null
    )?.metadata;
    if (Array.isArray(sourceMetadata?.activeProductIds)) {
      const ids = sourceMetadata.activeProductIds.filter(
  ...
  try {
    ...
    if (!parsed.success) {
      return NextResponse.json(
        { success: false, error: "Richiesta chat non valida" },
        { status: 400 },
      );
    }
    ...
    const pageContext =
      body.pageContext &&
      pageContextMatchesOrigin(body.pageContext, requestOrigin)
        ? body.pageContext
        : undefined;
    if (body.source === "widget") {
      if (!body.userSessionId) {
        return NextResponse.json(
          { success: false, error: "widget_session_required" },
          { status: 401 },
        );
      }
      try {
      ...
      const rate = await checkRateLimit(
        `widget-chat:${botId}:${requestClientIp(request.headers)}`,
        30,
        60 * 1000,
      );
      if (!rate.allowed) {
      ...
      const publication = await prisma.chatbot.findUnique({
        where: { id: botId },
          ...
          data: {
            ...
            sourcesUsed: stringifyJSON({
              ...
              metadata: {
                ...
                verified: orderLookup.verified,
                provider: orderLookup.provider,
                capability: orderLookup.capability,
              },
            }),
          },
        }),
      ...
      return NextResponse.json({
        ...
        data: {
          ...
          intent: {
            type: "order_tracking",
            confidence: 1,
            reasoning: `Flusso deterministico con verifica ${orderLookup.provider || "commerce"}`,
          },
          queryClassification: { type: "transactional", complexity: "simple" },
          decision: {
            strategy: "verified_order_lookup",
            sources: orderLookup.provider ? [orderLookup.provider] : [],
            reasoning: "Verifica diretta sul negozio collegato",
          },
          confidence: {
            score: orderLookup.verified ? 1 : 0.8,
            isCoherent: true,
          },
          ...
          actions: {
            ...
            leadForms: [],
            channelMessages: [],
            handoffActivated: Boolean(orderLookup.handoff),
          },
          quickReplies: [],
          ctas: [],
          productCards: [],
    ...
    console.log(
      `🧠 [ChatAPI] Optimizing conversation context (${conversation.messages.length} messages)`,
    );
    ...
    const { contextMessages, summary, wasSummarized, usedTokens } =
      await getOptimizedContext(allMessages, {
        maxMessages: 8,
        maxTokens: 3000,
    ...
    if (wasSummarized) {
      console.log(
        `📝 [ChatAPI] Conversation summarized (${allMessages.length} → ${contextMessages.length} messages)`,
      );
      console.log(
        `📊 [ChatAPI] Token usage: ${formatTokenUsage(usedTokens || 0, 4096)}`,
      );
      ...
      console.log(
        `📊 [ChatAPI] Token usage: ${formatTokenUsage(usedTokens, 4096)}`,
      );
    }
      ...
      classifiedCommerceIntent !== "none"
        ...
        : catalogFollowUpQuery
          ...
          : activeProductIds.length > 0 && parsedCommerceQuery.wantsCards
            ? "product_discovery"
            : "none";
    const productSearch = await searchVerifiedProducts(
      botId,
      commerceSearchMessage,
      pageContext,
      {
    ...
    if (
      ...
      (needsStyleClarification ||
        needsProductClarification ||
        (requiresCatalog && productSearch.selections.length === 0))
    ) {
      ...
      const assistantMessage = await prisma.message.create({
        data: {
          ...
          sourcesUsed: stringifyJSON({
            sources: [],
            metadata: { responseType, verified: true },
          }),
      ...
      return NextResponse.json({
        ...
        data: {
          ...
          sources: [],
          intent: {
            type: needsStyleClarification ? "fit_advice" : "product_search",
            confidence: 1,
            reasoning: needsStyleClarification
              ? "Consiglio outfit: richiesta di dettaglio prima di proporre prodotti"
              : needsProductClarification
                ? "Scoperta esigenze prima di proporre prodotti"
                : "Richiesta prodotto vincolata al catalogo verificato",
          },
          queryClassification: { type: "transactional", complexity: "simple" },
          decision: {
            strategy: "verified_catalog_guard",
            sources: ["product_catalog"],
            reasoning: "Nessun prodotto verificato corrispondente",
          },
    ...
    const workflowResult =
      incomingPolicy.action === "allow" && !groundingBlocked
        ? await import("@/lib/workflow-engine").then(({ runActiveWorkflows }) =>
            runActiveWorkflows({
              botId,
              conversationId: conversation.id,
              messageId: userMessage.id,
    ...
    if (workflowResult.responseOverride)
      result.response = workflowResult.responseOverride;
    // Deterministic actions (lead form, appointment, order lookup) do not depend
    // on RAG evidence. Their own schemas, URL allowlists and runtime checks are
    // the source of truth, so a knowledge fallback must not silently hide them.
    const actionResult =
      incomingPolicy.action === "allow"
        ? await import("@/lib/action-engine").then(({ runTriggeredActions }) =>
            runTriggeredActions({
              botId,
              conversationId: conversation.id,
              messageId: userMessage.id,
    ...
    const policyDecision =
      incomingPolicy.action !== "allow" ? incomingPolicy : outgoingPolicy;
    ...
    const productCardLimit = actionResult.forceProductCards
      ? Math.max(productSearch.query.maxCards, 5)
      : productSearch.query.maxCards;
    const productCards =
      productSearch.query.wantsCards || actionResult.forceProductCards
        ? resolvedProductCards.slice(0, productCardLimit)
        : [];
    if (requiresCatalog && resolvedProductCards.length > 0) {
      result.response = buildVerifiedProductResponse(
    ...
    console.log(
      `✅ [ChatAPI] Orchestrator completed in ${result.metadata.processingTimeMs}ms`,
    );
    ...
    console.log(
      `💡 [ChatAPI] UX Enhancements: ${quickReplies.length} quick replies, ${contextualCTAs.length} CTAs`,
    );
    ...
    const messageMetadata = {
      intent: effectiveIntent,
      intentConfidence:
        commerceIntent !== "none" ? 1 : result.decision.intent.confidence,
    ...
    const existingTopics = latestConversation?.topicsDiscussed
      ? JSON.parse(latestConversation.topicsDiscussed)
      : [];
    const topics = Array.from(
      new Set([...existingTopics, ...result.decision.topics]),
    );
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        intent: {
          ...
          reasoning:
            commerceIntent !== "none"
              ? "Intento commerce deterministico"
        ...
        memory: {
          persistentFactsUsed:
            result.retrievalResult?.persistentFacts.length || 0,
          knowledgeChunksUsed:
            result.retrievalResult?.knowledgeChunks.length || 0,
          factsExtracted: result.extractedFacts.length,
          factsExtractionScheduled:
            result.metadata.phaseTimings.learningScheduled,
        },
