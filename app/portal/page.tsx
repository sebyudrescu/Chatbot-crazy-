...
import { BarChart3, Bot, Building2, Copy, Database, KeyRound, Loader2, LogOut, MessageSquareText, RefreshCcw, Settings, ShieldCheck, Trash2, UserPlus, Users } from 'lucide-react'
...
type Agent = { id: string; workspaceId: string; companyName: string; isActive: boolean; kbStatus: string; _count: { conversations: number; knowledgeSources: number } }
...
export default function ClientPortalPage() {
  ...
  return <main className="min-h-screen bg-gray-50"><header className="border-b border-gray-200 bg-white"><div className="mx-auto flex max-w-6xl items-center justify-between px-5 py-4"><div className="flex items-center gap-3"><div className="flex h-10 w-10 items-center justify-center rounded-xl bg-brand-600 text-white"><Bot className="h-5 w-5" /></div><div><p className="font-bold text-gray-950">LitX AI</p><p className="text-[10px] uppercase tracking-widest text-gray-400">Portale cliente</p></div></div><div className="flex items-center gap-2"><Link href="/account/security" className="btn btn-secondary inline-flex items-center gap-2"><KeyRound className="h-4 w-4"/><span className="hidden sm:inline">Sicurezza</span></Link><Button variant="secondary" onClick={logout} icon={<LogOut className="h-4 w-4" />}>Esci</Button></div></div></header><div className="mx-auto max-w-6xl px-5 py-8"><div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="text-sm text-gray-500">Bentornato, {account!.displayName}</p><h1 className="mt-1 text-3xl font-bold text-gray-950">Il tuo spazio operativo</h1></div><div className="flex items-center gap-2 rounded-xl border border-emerald-200 bg-emerald-50 px-3 py-2 text-xs font-semibold text-emerald-700"><ShieldCheck className="h-4 w-4" />Dati isolati per azienda</div></div><div className="mt-7 grid gap-4 md:grid-cols-3"><Metric icon={<MessageSquareText />} label="Conversazioni · 30 giorni" value={analytics.conversations} /><Metric icon={<BarChart3 />} label="Messaggi · 30 giorni" value={analytics.messages} /><Metric icon={<Building2 />} label="Lead identificati" value={analytics.leads} /></div><section className="mt-8"><div className="flex items-center justify-between"><div><h2 className="text-lg font-bold text-gray-950">I tuoi agenti</h2><p className="mt-1 text-xs text-gray-500">Visualizzi esclusivamente gli agenti dei workspace a cui sei stato invitato.</p></div></div><div className="mt-4 grid gap-4 md:grid-cols-2">{agents.map(agent => { const role = account!.memberships.find(membership => membership.workspace.id === agent.workspaceId)?.role; const canConfigure = role === 'owner' || role === 'admin'; return <article key={agent.id} className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm"><div className="flex items-start justify-between gap-3"><div><h3 className="font-bold text-gray-950">{agent.companyName}</h3><p className="mt-1 text-xs text-gray-500">Knowledge base: {agent.kbStatus}</p></div><span className={`rounded-full px-2.5 py-1 text-[10px] font-bold ${agent.isActive ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-700'}`}>{agent.isActive ? 'Attivo' : 'Bozza'}</span></div><div className="mt-5 grid grid-cols-2 gap-3"><div className="rounded-xl bg-gray-50 p-3"><p className="text-xl font-bold text-gray-950">{agent._count.conversations}</p><p className="text-[10px] uppercase tracking-wide text-gray-400">Conversazioni</p></div><div className="rounded-xl bg-gray-50 p-3"><p className="text-xl font-bold text-gray-950">{agent._count.knowledgeSources}</p><p className="text-[10px] uppercase tracking-wide text-gray-400">Fonti</p></div></div><div className="mt-4 grid grid-cols-2 gap-2"><PortalLink href={`/analytics?botId=${agent.id}`} icon={<BarChart3 />} label="Analytics" /><PortalLink href={`/conversations?botId=${agent.id}`} icon={<MessageSquareText />} label="Chat e Help Desk" />{canConfigure && <PortalLink href={`/knowledge?botId=${agent.id}`} icon={<Database />} label="Knowledge" />}{canConfigure && <PortalLink href={`/chatbot/${agent.id}/settings`} icon={<Settings />} label="Configura" />}</div></article> })}{agents.length === 0 && <div className="rounded-2xl border border-dashed border-gray-300 bg-white p-8 text-center text-sm text-gray-500">Nessun agente assegnato a questo account.</div>}</div></section>
...
function PortalLink({ href, icon, label }: { href: string; icon: React.ReactNode; label: string }) {
  return <Link href={href} className="flex items-center gap-2 rounded-xl border border-gray-200 px-3 py-2 text-xs font-semibold text-gray-700 transition hover:border-brand-200 hover:bg-brand-50 hover:text-brand-700"><span className="text-brand-600">{icon}</span>{label}</Link>
}
