...
const STOP_WORDS = new Set([
  ...
  "avete", "vendete", "cerco", "cercando", "serve", "servirebbe", "farmi", "vedere", "mostrami", "mostrarmi", "propormi", "altri",
  "sotto", "entro", "massimo", "euro", "eur", "prezzo", "prezzi", "taglia", "taglie", "colore", "colori", "numero",
  "reale", "reali", "realmente", "adesso", "disponibile", "disponibili", "link", "pagina", "pagine", "esatto", "esatta", "esatti", "esatte",
  "fammi", "dettaglio", "dettagli", "scheda", "schede", "solo", "senza", "non",
...
function tokens(query: string) {
  return [...new Set(normalizeCommerceText(query).split(/\s+/).filter((token) => token.length >= 3 && !STOP_WORDS.has(token)))].slice(0, 12);
}
...
function variantOptionSummary(variants: Array<{ attributes: string; available: boolean }>) {
  const values = new Map<string, { available: Set<string>; unavailable: Set<string> }>();
  for (const variant of variants) {
    for (const [name, value] of Object.entries(jsonRecord(variant.attributes))) {
      const entry = values.get(name) ?? { available: new Set<string>(), unavailable: new Set<string>() };
      (variant.available ? entry.available : entry.unavailable).add(value);
      values.set(name, entry);
    }
  }
  return [...values.entries()].map(([name, entry]) => {
    const available = [...entry.available];
    const unavailable = [...entry.unavailable].filter((value) => !entry.available.has(value));
...
export async function searchVerifiedProducts(
  ...
  const isCatalogIntent = ["product_discovery", "product_detail", "variant_availability", "product_comparison", "fit_advice"].includes(parsed.intent);
  ...
  const [products, catalogSize] = await Promise.all([prisma.product.findMany({
    ...
    where: { botId, status: "active", recommendationStatus: { notIn: ["excluded", "blocked"] } },
  })]);
  ...
  const ranked = products.flatMap((product) => {
    ...
    const variantAttributes = product.variants.flatMap((variant) => Object.entries(jsonRecord(variant.attributes)).flat()).join(" ");
    const structured = normalizeCommerceText([product.title, product.brand, product.productType, ...jsonStrings(product.categories), ...jsonStrings(product.tags), variantAttributes].filter(Boolean).join(" "));
    const descriptive = normalizeCommerceText(`${structured} ${product.description}`);

    const availableVariants = product.variants.filter((variant) => variant.available);
    if (!matchesCommerceConstraints(parsed, {
      structuredText: structured,
      descriptiveText: descriptive,
      availableForSale: product.availableForSale,
      availablePrices: availableVariants.flatMap((variant) => variant.price === null ? [] : [variant.price]),
      availableOptionValues: availableVariants.flatMap((variant) => Object.values(jsonRecord(variant.attributes))),
    })) return [];

    let eligibleVariants = availableVariants;
    if (parsed.minPrice !== undefined) eligibleVariants = eligibleVariants.filter((variant) => variant.price !== null && variant.price >= parsed.minPrice!);
    if (parsed.maxPrice !== undefined) eligibleVariants = eligibleVariants.filter((variant) => variant.price !== null && variant.price <= parsed.maxPrice!);
    if (parsed.size) eligibleVariants = eligibleVariants.filter((variant) => {
      const attrs = Object.values(jsonRecord(variant.attributes)).map((value) => normalizeCommerceText(value));
      return attrs.includes(normalizeCommerceText(parsed.size!)) || normalizeCommerceText(variant.title || "") === normalizeCommerceText(parsed.size!);
    });
    if (product.variants.length > 0 && eligibleVariants.length === 0) return [];

    let variant = pageContext?.sku
      ? eligibleVariants.find((item) => item.sku?.toLowerCase() === pageContext.sku?.toLowerCase())
      : eligibleVariants[0] ?? product.variants.find((item) => item.available) ?? product.variants[0];
    if (pageContext?.sku && !variant) return [];

    let score = 20 + product.rankingBoost;
    const now = new Date();
    const campaignActive = (!product.campaignStart || product.campaignStart <= now) && (!product.campaignEnd || product.campaignEnd >= now);
    if (product.recommendationStatus === "promoted" && campaignActive) score += 30;
    if (pageContext?.productId && product.externalId === pageContext.productId) score += 500;
    if (pageContext?.sku && variant?.sku?.toLowerCase() === pageContext.sku.toLowerCase()) score += 500;
    if (activeProductIds.includes(product.id)) score += 250;
    if (title.length >= 4 && normalizedQuery.includes(title)) score += 600;

    let lexicalMatches = 0;
    for (const token of queryTokens) {
      if (containsToken(title, token)) { score += 35; lexicalMatches++; }
      else if (containsToken(structured, token)) { score += 18; lexicalMatches++; }
      else if (containsToken(descriptive, token)) { score += 4; lexicalMatches++; }
      if (product.variants.some((item) => item.sku && containsToken(normalizeCommerceText(item.sku), token))) { score += 35; lexicalMatches++; }
    }
    if (parsed.category) score += 80;
    if (parsed.colors.length) score += 45;
    if (parsed.materials.length) score += 35;
    if (parsed.gender) score += 20;
    if (!parsed.category && lexicalMatches === 0 && !activeProductIds.includes(product.id)) return [];
  ...
  }).sort((a, b) => b.score - a.score || a.product.title.localeCompare(b.product.title, "it"))
    .slice(0, parsed.maxCards > 0 ? Math.max(parsed.maxCards, parsed.intent === "product_comparison" ? 2 : 1) : 1);

  const selections = ranked.map(({ product, variant, reason }) => ({ productId: product.id, variantId: variant?.id, reason }));
