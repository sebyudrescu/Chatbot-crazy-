...
export default function CommercePage() {
  ...
  const loadCatalog = useCallback(async () => {
    ...
    try {
      const response = await fetch(`/api/commerce?botId=${encodeURIComponent(botId)}`)
  ...
  const products = data?.products.filter(product => !deferredSearch || [product.title, product.brand, ...product.variants.map(variant => variant.sku)].some(value => value?.toLowerCase().includes(deferredSearch))) || []
    ...
    <DashboardLayout>
      <div className="mx-auto max-w-[1500px] p-4 lg:p-7">
        <div className="flex flex-col justify-between gap-4 md:flex-row md:items-end">
          ...
          <div className="flex gap-2"><select value={botId} onChange={event => setBotId(event.target.value)} className="input min-w-52 py-2 text-xs" aria-label="Seleziona agente">{agents.map(agent => <option key={agent.id} value={agent.id}>{agent.companyName}</option>)}</select><Button variant="secondary" size="sm" onClick={loadCatalog} icon={<RefreshCw className="h-4 w-4" />}>Aggiorna</Button></div>
        </div>
        ...
        <div className="mt-5 grid gap-5 xl:grid-cols-[minmax(0,1fr)_300px]">
          <Card padding="none" className="overflow-hidden">
            <div className="flex flex-col gap-3 border-b border-gray-100 p-4 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="text-sm font-semibold text-gray-950">Prodotti verificati</h2><p className="mt-1 text-[10px] text-gray-400">Il modello non può modificare link, immagini, prezzo o stock.</p></div><input value={search} onChange={event => setSearch(event.target.value)} placeholder="Cerca titolo, brand o SKU…" className="input max-w-sm py-2 text-xs" aria-label="Cerca nel catalogo" /></div>
