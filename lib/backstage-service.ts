...
const TOOLS = [
  ...
  { type: "function", name: "create_draft", description: "Crea una bozza persistente, modificabile e NON applicata. Nessuna modifica live avviene con questo tool.", strict: true, parameters: { type: "object", properties: { type: { type: "string", enum: BackstageDraftTypeSchema.options }, title: { type: "string" }, summary: { type: "string" }, payload_json: { type: "string", description: "JSON valido. action: campi Action senza botId/enabled e targetId opzionale per modificare; workflow: campi Workflow senza botId/isActive e targetId opzionale; prompt: {systemPrompt?,settingsPatch?}; knowledge_url:{url,crawlSite}; evaluations:{cases:[...]}" }, evidence: { type: "array", items: { type: "string" } } }, required: ["type", "title", "summary", "payload_json", "evidence"], additionalProperties: false } },
] as const;
  ...
  await prisma.$transaction(async tx => {
    if (type === "action") {
      ...
      for (const item of payload.cases) {
        const created = await tx.evaluationCase.create({ data: { botId: draft.botId, ...item, expectedKeywords: JSON.stringify(item.expectedKeywords), forbiddenKeywords: JSON.stringify(item.forbiddenKeywords), isActive: true } });
        ids.push(created.id);
      }
