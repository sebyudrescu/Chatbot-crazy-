...
export async function POST(request: NextRequest, props: { params: Promise<{ id: string }> }) {
  try {
    ...
    const cloned = await prisma.$transaction(async tx => {
      const agent = await tx.chatbot.create({ data: { companyName: input.companyName || `${source.companyName} — Copia`, isActive: false, settings: source.settings, promptTemplateId: source.promptTemplateId, systemPrompt: source.systemPrompt, promptVariables: source.promptVariables, kbStatus: 'empty', kbTotalChunks: 0 } })
