...
export default function EvaluationsPage() {
  ...
  const averageMetric = (path: string[]) => { const values = latestRuns.map(run => path.reduce<any>((value, key) => value?.[key], run.metrics)).filter((value): value is number => typeof value === 'number'); return values.length ? Math.round(values.reduce((sum, value) => sum + value, 0) / values.length * 100) : null }
  const faithfulness = averageMetric(['faithfulness'])
  const answerAccuracy = averageMetric(['answerAccuracy'])
  const precisionAtFive = averageMetric(['retrieval', 'precisionAtK'])
  const recallAtFive = averageMetric(['retrieval', 'recallAtK'])
  const reciprocalRank = averageMetric(['retrieval', 'reciprocalRank'])
  ...
  return <DashboardLayout><div className="mx-auto max-w-[1500px] p-4 lg:p-7">
    ...
    {!agents.length ? <div className="mt-6 card flex min-h-80 flex-col items-center justify-center text-center"><Bot className="h-8 w-8 text-brand-600" /><h2 className="mt-3 font-semibold">Nessun agente disponibile</h2><p className="mt-1 text-sm text-gray-500">Crea un agente per definire i suoi controlli automatici.</p></div> : <>
      <div className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><Metric label="Agente" value={selected?.companyName || '—'} /><Metric label="Successo ultimo run" value={`${passRate}%`} good={passRate >= 80} /><Metric label="Faithfulness" value={metricDisplay(faithfulness)} good={metricQuality(faithfulness, 70)} /><Metric label="Accuratezza risposta" value={metricDisplay(answerAccuracy)} good={metricQuality(answerAccuracy, 70)} /><Metric label="Precision@5" value={metricDisplay(precisionAtFive)} good={metricQuality(precisionAtFive, 60)} /><Metric label="Recall@5" value={metricDisplay(recallAtFive)} good={metricQuality(recallAtFive, 70)} /><Metric label="MRR" value={metricDisplay(reciprocalRank)} good={metricQuality(reciprocalRank, 70)} /><Metric label="Esecuzioni salvate" value={String(runs.length)} /></div>
