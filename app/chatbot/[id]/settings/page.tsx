...
interface AgentSettings {
  ...
  retrievalMinScore?: number; groundingThreshold?: number; rerankerEnabled?: boolean
  liveWebSearchEnabled?: boolean; liveWebAllowedDomains?: string[]
  ragCalibration?: { retrievalMinScore: number; groundingThreshold: number; sampleCount: number; calibratedAt: string }
...
const defaults: Required<Pick<AgentSettings, 'role' | 'objective' | 'personality' | 'rules' | 'forbiddenTopics' | 'forbiddenResponses' | 'handoffTriggers' | 'leadCollectionFields' | 'language' | 'tone' | 'responseLength' | 'fallbackMessage' | 'handoffMessage' | 'aiModel' | 'temperature' | 'maxTokens' | 'retrievalMinScore' | 'groundingThreshold' | 'rerankerEnabled' | 'liveWebSearchEnabled' | 'liveWebAllowedDomains'>> = {
    ...
    <div className="mx-auto max-w-[1500px] px-5 py-6 lg:px-7">
      ...
      <div className="grid gap-5 xl:grid-cols-[190px_minmax(0,1fr)_350px]">
        ...
        <div className="space-y-5">
          ...
          <section id="section-welcome" className="card p-5"><div className="flex items-start gap-3"><div className="rounded-xl bg-brand-50 p-2 text-brand-700"><MessageSquare className="h-5 w-5" /></div><div><p className="eyebrow">Prima impressione</p><h2 className="mt-1 text-base font-semibold text-gray-950">Messaggio iniziale</h2><p className="mt-1 text-xs text-gray-500">È il primo messaggio mostrato all’apertura della chat. Puoi scrivere liberamente oppure partire da un esempio.</p></div></div><div className="mt-4 grid gap-2 sm:grid-cols-3">{[
          ...
          ].map((message, index) => <button key={message} type="button" onClick={() => update('welcomeMessage', message)} className={`rounded-xl border p-3 text-left text-[11px] leading-4 transition-colors ${settings.welcomeMessage === message ? 'border-brand-400 bg-brand-50 text-brand-800' : 'border-gray-200 bg-white text-gray-600 hover:border-brand-200 hover:bg-brand-50/40'}`}><span className="mb-1 block text-[9px] font-semibold uppercase tracking-wider text-brand-600">Esempio {index + 1}</span>{message}</button>)}</div><Field label="Il tuo welcome message" className="mt-4"><textarea value={settings.welcomeMessage || ''} onChange={e => update('welcomeMessage', e.target.value.slice(0, 500))} rows={4} maxLength={500} placeholder={defaultWelcomeMessage(bot.companyName)} className="textarea" /><div className="mt-1 flex items-center justify-between gap-3 text-[10px] text-gray-400"><button type="button" onClick={() => update('welcomeMessage', '')} className="font-medium text-brand-600 hover:text-brand-700">Ripristina predefinito</button><span>{(settings.welcomeMessage || '').length} / 500</span></div></Field></section>
          ...
          <section id="section-rag" className="card p-5"><p className="eyebrow">Qualità RAG</p><h2 className="mt-1 text-base font-semibold text-gray-950">Retrieval, grounding e ricerca controllata</h2><div className="mt-4 grid gap-4 md:grid-cols-2"><Field label={`Soglia retrieval · ${Math.round((settings.retrievalMinScore || 0) * 100)}%`}><input aria-label="Soglia retrieval" type="range" min="0" max="1" step="0.01" value={settings.retrievalMinScore} onChange={e => update('retrievalMinScore', Number(e.target.value))} className="mt-3 w-full accent-brand-600" /></Field><Field label={`Soglia grounding · ${Math.round((settings.groundingThreshold || 0) * 100)}%`}><input aria-label="Soglia grounding" type="range" min="0" max="1" step="0.01" value={settings.groundingThreshold} onChange={e => update('groundingThreshold', Number(e.target.value))} className="mt-3 w-full accent-brand-600" /></Field></div><div className="mt-4 grid gap-3 md:grid-cols-2"><Toggle label="Cross-encoder reranker" description="Riordina i risultati con un modello multilingua e usa il ranking locale come fallback." checked={Boolean(settings.rerankerEnabled)} onChange={value => update('rerankerEnabled', value)} /><Toggle label="Ricerca web autorizzata" description="Consulta soltanto i domini elencati e mostra sempre le citazioni." checked={Boolean(settings.liveWebSearchEnabled)} onChange={value => update('liveWebSearchEnabled', value)} /></div><Field label="Domini web autorizzati (uno per riga)" className="mt-4"><textarea value={(settings.liveWebAllowedDomains || []).join('\n')} onChange={e => update('liveWebAllowedDomains', e.target.value.split(/\r?\n|,/).map(value => value.trim()).filter(Boolean))} rows={3} placeholder={'esempio.it\nshop.esempio.it'} className="textarea font-mono text-xs" /></Field>{settings.ragCalibration && <p className="mt-3 rounded-lg bg-emerald-50 px-3 py-2 text-[10px] text-emerald-700">Soglie calibrate su {settings.ragCalibration.sampleCount} casi il {new Date(settings.ragCalibration.calibratedAt).toLocaleString('it-IT')}.</p>}</section>
        </div>

        <aside className="space-y-5 xl:sticky xl:top-20 xl:h-fit">
          <div className="card overflow-hidden"><div className="border-b border-gray-100 px-4 py-3"><p className="text-xs font-semibold text-gray-900">Anteprima agente</p></div><div className="p-4"><div className="overflow-hidden rounded-2xl border border-gray-200 shadow-soft"><div className="flex items-center gap-2 bg-gradient-to-r from-brand-700 to-brand-500 px-4 py-3 text-white"><div className="flex h-8 w-8 items-center justify-center rounded-full bg-white/20"><Sparkles className="h-4 w-4" /></div><div><p className="text-xs font-semibold">{settings.botName || bot.companyName}</p><p className="text-[9px] text-white/75">Online</p></div></div><div className="min-h-56 bg-gray-50 p-4"><div className="max-w-[85%] whitespace-pre-wrap rounded-xl rounded-tl-sm bg-white p-3 text-xs leading-5 text-gray-700 shadow-sm">{settings.welcomeMessage?.trim() || defaultWelcomeMessage(bot.companyName)}</div><div className="mt-3 flex flex-wrap gap-2"><span className="rounded-lg border bg-white px-2.5 py-1.5 text-[10px] text-brand-700">Informazioni</span><span className="rounded-lg border bg-white px-2.5 py-1.5 text-[10px] text-brand-700">Parla con il team</span></div></div><div className="flex items-center gap-2 border-t bg-white p-3"><span className="flex-1 text-[10px] text-gray-400">Scrivi qui il tuo messaggio...</span><MessageSquare className="h-4 w-4 text-brand-600" /></div></div></div></div>
...
function Toggle({ label, description, checked, onChange }: { label: string; description: string; checked: boolean; onChange: (value: boolean) => void }) { return <button type="button" aria-pressed={checked} onClick={() => onChange(!checked)} className={`flex items-start justify-between gap-3 rounded-xl border p-3 text-left ${checked ? 'border-brand-200 bg-brand-50' : 'border-gray-200 bg-white'}`}><span><span className="block text-xs font-semibold text-gray-800">{label}</span><span className="mt-1 block text-[10px] leading-4 text-gray-500">{description}</span></span><span className={`mt-0.5 h-5 w-9 rounded-full p-0.5 transition-colors ${checked ? 'bg-brand-600' : 'bg-gray-200'}`}><span className={`block h-4 w-4 rounded-full bg-white transition-transform ${checked ? 'translate-x-4' : ''}`} /></span></button> }
