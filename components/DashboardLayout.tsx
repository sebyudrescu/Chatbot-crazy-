...
type DashboardAccount =
  ...
  | {
      ...
      displayName: string;
      email: string;
      memberships: Array<{ role: "owner" | "admin" | "operator" | "viewer" }>;
    };
...
export function DashboardLayout({ children }: DashboardLayoutProps) {
  ...
  useEffect(() => {
    fetch("/api/auth/me", { cache: "no-store" })
      ...
      .then((result) =>
        setDashboardAccount(result.success ? result.data : null),
      )
  ...
  const clientCanConfigure =
    clientMode &&
    dashboardAccount.memberships.some(
      (membership) =>
        membership.role === "owner" || membership.role === "admin",
    );
  const accountName = clientMode
    ? dashboardAccount.displayName
    : ownerMode
      ? "Sebastian U."
      : "LitX AI";
  const accountDetail = clientMode
    ? dashboardAccount.email
    : ownerMode
      ? "Amministratore"
      : "Caricamento account…";
  const accountInitials = clientMode
    ? dashboardAccount.displayName.slice(0, 2).toUpperCase()
    : ownerMode
      ? "SU"
      : "LX";
  const accountFooter = (
    <div className="flex items-center gap-3 rounded-xl border border-gray-100 bg-gray-50 p-2.5">
      <div className="flex h-8 w-8 items-center justify-center rounded-full bg-gray-950 text-[10px] font-semibold text-white">
        {accountInitials}
      </div>
      ...
      <button
        ...
        title="Esci"
        className="rounded-lg p-2 text-gray-400 transition hover:bg-white hover:text-red-600"
      >
  ...
  return (
    <div className="flex min-h-screen bg-[#f8f8fb]">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-bold tracking-tight text-gray-950">
                LitX <span className="text-brand-600">AI</span>
              </h1>
      ...
      >
        {clientMode ? (
          <ClientNavigation canConfigure={clientCanConfigure} />
        ) : ownerMode ? (
          <OwnerNavigation />
        ) : null}
      </Sidebar>
      ...
      {mobileNavigationOpen && (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <aside
            ...
            className="relative flex h-full w-[min(86vw,320px)] flex-col border-r border-gray-200 bg-white shadow-2xl"
          >
            <div className="flex items-center justify-between border-b border-gray-100 px-5 py-4">
              ...
              <button
                ...
                className="rounded-lg p-2 text-gray-500 transition hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-brand-500"
              >
                <X className="h-5 w-5" />
              </button>
            </div>
            <nav className="flex-1 space-y-5 overflow-y-auto px-3 py-4">
              <MobileNavigation
                ...
                ownerMode={ownerMode}
                clientCanConfigure={clientCanConfigure}
              />
            </nav>
        ...
        <header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-[#ecebf1] bg-white/95 px-4 backdrop-blur lg:px-7">
          ...
          <div className="ml-2 flex shrink-0 items-center gap-2 lg:ml-4">
            ...
            {ownerMode ? (
              ...
              >
                <span className="hidden md:inline">Nuovo Agente</span>
              </Button>
            ) : null}
            <div className="ml-1 hidden h-9 w-9 items-center justify-center rounded-full bg-gradient-to-br from-gray-800 to-gray-950 text-xs font-semibold text-white sm:flex">
              {accountInitials}
            </div>
          </div>
        </header>
...
function MobileNavigation({
  ...
  clientMode: boolean;
  ownerMode: boolean;
  clientCanConfigure: boolean;
}) {
  if (clientMode)
    return (
      <ClientNavigation
        canConfigure={clientCanConfigure}
...
function OwnerNavigation({ onNavigate }: { onNavigate?: () => void }) {
  return (
    <>
      <SidebarSection title="Clienti e chatbot">
        <SidebarLink
          onClick={onNavigate}
          href="/dashboard"
          icon={BarChart3}
      ...
      <SidebarSection title="Configurazione">
        <SidebarLink
          onClick={onNavigate}
          href="/templates"
          icon={LayoutTemplate}
      ...
      <SidebarSection title="Operatività">
        <SidebarLink
          onClick={onNavigate}
          href="/conversations"
          icon={MessageSquare}
      ...
      <SidebarSection title="Qualità e AI">
        <SidebarLink
          onClick={onNavigate}
          href="/testing"
          icon={FlaskConical}
