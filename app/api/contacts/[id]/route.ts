...
export async function PATCH(request: NextRequest, props: { params: Promise<{ id: string }> }) {
  ...
  try {
    const actor=await requireDashboardActor(request);await requireResourcePermission(actor,'contact',params.id,'chatbot.write')
    ...
    const { tags, note, email, phone, ...fields } = input
    const notes = note ? [...parse(current.notes), { id: crypto.randomUUID(), text: note, createdAt: new Date().toISOString() }] : undefined
    const updated = await prisma.cRMContact.update({ where: { id: params.id }, data: { ...fields, email: email === undefined ? undefined : normalizeCRMEmail(email), phone: phone === undefined ? undefined : normalizeCRMPhone(phone), tags: tags ? JSON.stringify([...new Set(tags.map(tag => tag.toLowerCase()))]) : undefined, notes: notes ? JSON.stringify(notes) : undefined } })
