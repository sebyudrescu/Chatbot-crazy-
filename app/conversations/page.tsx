...
import { Bot, Check, Clock3, Download, Mail, MessageSquare, Phone, Plus, RefreshCw, Search, Send, Sparkles, StickyNote, Tag as TagIcon, User, UserRoundCheck, X } from 'lucide-react'
...
interface Conversation {
  ...
  chatbot: { id: string; companyName: string }; messages: Message[]; _count: { messages: number }
}
...
export default function ConversationsPage() {
  ...
  const openConversation = useCallback(async (conversation: Conversation) => {
    ...
    try {
      ...
      if (result.success) {
        const full = { ...result.data, _count: conversation._count } as Conversation
        setSelected(full)
        setNoteDraft(full.internalNotes || '')
        setItems(current => current.map(item => item.id === full.id ? { ...item, ...full } : item))
      }
  ...
  const load = useCallback(async () => {
    try {
      ...
      if (conversations[0] && !selected) {
        const requestedId = typeof window !== 'undefined' ? new URLSearchParams(window.location.search).get('conversation') : null
        await openConversation(conversations.find((item: Conversation) => item.id === requestedId) || conversations[0])
      }
  ...
  const filtered = useMemo(() => items.filter(item => {
    ...
    const matchStatus = status === 'all' || (status === 'open' && !item.isResolved) || (status === 'resolved' && item.isResolved) || (status === 'escalated' && item.needsHumanEscalation)
  ...
  const patchSelected = async (data: Partial<Conversation>) => {
    ...
    setBusy(true)
    try {
      const response = await fetch(`/api/conversations/${selected.id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data) })
      const result = await response.json()
      if (!response.ok) throw new Error(result.error)
      const updated = { ...selected, ...result.data }
      setSelected(updated); setItems(current => current.map(item => item.id === updated.id ? { ...item, ...updated } : item))
  ...
  const assist = async (mode: 'summary' | 'reply') => {
    ...
    try {
      const response = await fetch(`/api/conversations/${selected.id}/assist`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ mode }) })
      const result = await response.json()
      if (!response.ok) throw new Error(result.error || 'Assistenza AI non riuscita')
      if (mode === 'reply') setReply(result.data.suggestedReply)
      else await patchSelected({ summary: result.data.summary, tags: [...new Set([...(selected.tags || []), ...result.data.tags])] })
    } catch (error) { setAiError(error instanceof Error ? error.message : 'Assistenza AI non riuscita') }
    finally { setAiBusy(null) }
  }

  const addTag = async () => {
    if (!selected || !tagDraft.trim()) return
    await patchSelected({ tags: [...new Set([...(selected.tags || []), tagDraft.trim().toLowerCase()])] })
    setTagDraft('')
  }

  const removeTag = (tag: string) => selected && patchSelected({ tags: selected.tags.filter(item => item !== tag) })

  if (loading) return <DashboardLayout><LoadingSpinner fullPage text="Caricamento inbox..." /></DashboardLayout>

  return <DashboardLayout>
    <div className="flex h-[calc(100vh-4rem)] min-h-[700px] overflow-hidden">
      <aside className="flex w-[350px] shrink-0 flex-col border-r border-gray-200 bg-white">
        <div className="border-b p-5"><div className="flex items-center justify-between"><div><p className="eyebrow">Inbox operativa</p><h1 className="mt-1 text-xl font-bold text-gray-950">Chat Logs</h1></div><div className="flex gap-1"><a href={`/api/conversations/export?botId=${encodeURIComponent(bot)}&status=${encodeURIComponent(status)}`} className="rounded-lg p-2 text-gray-400 hover:bg-gray-100" aria-label="Esporta conversazioni CSV" title="Esporta CSV"><Download className="h-4 w-4" /></a><button onClick={load} className="rounded-lg p-2 text-gray-400 hover:bg-gray-100" aria-label="Aggiorna"><RefreshCw className="h-4 w-4" /></button></div></div><div className="relative mt-4"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-gray-400" /><input value={search} onChange={event => setSearch(event.target.value)} placeholder="Cerca conversazioni..." className="input pl-9 text-xs" /></div><div className="mt-3 grid grid-cols-2 gap-2"><select value={bot} onChange={event => setBot(event.target.value)} className="input py-2 text-xs"><option value="all">Tutti gli agenti</option>{bots.map(item => <option key={item.id} value={item.id}>{item.companyName}</option>)}</select><select value={status} onChange={event => setStatus(event.target.value as Status)} className="input py-2 text-xs"><option value="all">Tutti gli stati</option><option value="open">Aperte</option><option value="escalated">Handoff</option><option value="resolved">Risolte</option></select></div></div>
        <div className="flex-1 overflow-y-auto">{filtered.map(item => <button key={item.id} onClick={() => openConversation(item)} className={`w-full border-b border-gray-100 p-4 text-left transition hover:bg-gray-50 ${selected?.id === item.id ? 'bg-brand-50/70' : ''}`}><div className="flex gap-3"><div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-gray-100 text-gray-600"><User className="h-4 w-4" /></div><div className="min-w-0 flex-1"><div className="flex items-center justify-between gap-2"><p className="truncate text-xs font-semibold text-gray-900">{displayName(item)}</p><span className="shrink-0 text-[9px] text-gray-400">{relativeDate(item.lastMessageAt || item.startedAt)}</span></div><p className="mt-0.5 truncate text-[10px] text-gray-400">{item.chatbot.companyName} · {item._count.messages} messaggi</p><div className="mt-2 flex gap-1.5">{item.needsHumanEscalation && <Tag color="red">Handoff</Tag>}{item.isResolved ? <Tag color="green">Risolta</Tag> : <Tag color="gray">Aperta</Tag>}{item.userIntent && <Tag color="brand">{item.userIntent}</Tag>}</div></div></div></button>)}{!filtered.length && <div className="p-8 text-center text-xs text-gray-400">Nessuna conversazione trovata.</div>}</div>
      </aside>
...
function displayName(item: Conversation) { return item.userName || item.userEmail || `Visitatore ${item.userSessionId.slice(-6)}` }
function relativeDate(value: string) { const hours = Math.floor((Date.now()-new Date(value).getTime())/36e5); return hours<1?'Ora':hours<24?`${hours}h`:new Date(value).toLocaleDateString('it-IT',{day:'2-digit',month:'short'}) }
function Tag({ children, color }: { children: React.ReactNode; color: 'red'|'green'|'gray'|'brand' }) { const classes={red:'bg-red-50 text-red-600',green:'bg-emerald-50 text-emerald-600',gray:'bg-gray-100 text-gray-500',brand:'bg-brand-50 text-brand-600'}; return <span className={`rounded px-1.5 py-0.5 text-[9px] font-semibold ${classes[color]}`}>{children}</span> }
function InfoRow({ icon: Icon, label, value }: { icon: typeof Bot; label: string; value: string }) { return <div className="flex gap-3"><div className="flex h-7 w-7 items-center justify-center rounded-lg bg-gray-50 text-gray-400"><Icon className="h-3.5 w-3.5" /></div><div className="min-w-0"><p className="text-[9px] uppercase tracking-wider text-gray-400">{label}</p><p className="truncate text-[11px] font-medium text-gray-700">{value}</p></div></div> }
function StateLine({ label, value, good }: { label:string; value:string; good?:boolean }) { return <div className="flex items-center justify-between text-[10px]"><span className="text-gray-400">{label}</span><span className={`font-semibold ${good===true?'text-emerald-600':good===false?'text-amber-600':'text-gray-700'}`}>{value}</span></div> }
function Divider(){return <div className="my-5 border-t"/>}
function MiniMetric({icon:Icon,label,value}:{icon:typeof Bot;label:string;value:string|number}){return <div className="rounded-lg border p-3"><Icon className="h-3.5 w-3.5 text-brand-600"/><p className="mt-2 text-[9px] text-gray-400">{label}</p><p className="text-xs font-semibold text-gray-800">{value}</p></div>}
function ContactEditor({ conversation, onSave }: { conversation:Conversation; onSave:(data:Partial<Conversation>)=>void }) { const [name,setName]=useState(conversation.userName||''); const [email,setEmail]=useState(conversation.userEmail||''); const [phone,setPhone]=useState(conversation.userPhone||''); return <div className="mt-4 space-y-3"><input className="input text-xs" placeholder="Nome" value={name} onChange={e=>setName(e.target.value)}/><input className="input text-xs" type="email" placeholder="Email" value={email} onChange={e=>setEmail(e.target.value)}/><input className="input text-xs" placeholder="Telefono" value={phone} onChange={e=>setPhone(e.target.value)}/><Button size="sm" fullWidth onClick={()=>onSave({userName:name||null,userEmail:email||null,userPhone:phone||null})}>Salva contatto</Button></div> }
