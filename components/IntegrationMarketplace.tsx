...
interface ShopifyStatus { configured: boolean; connected: boolean; status: string; shopDomain?: string | null; themeEditorUrl?: string | null; callbackUrl: string; webhookUrl: string; lastError?: string | null; lastSyncedAt?: string | null; orderTracking?: { requested: boolean; granted: boolean; pcdStatus: 'unknown' | 'required' | 'scope_required' } }
interface WooCommerceStatus { configured: boolean; connected: boolean; status: string; storeOrigin?: string | null; callbackUrl: string; webhookUrl: string; lastError?: string | null; lastSyncedAt?: string | null }
...
export function IntegrationMarketplace({ initialCategory = 'all' }: { initialCategory?: string }) {
  ...
  const [wooStatus, setWooStatus] = useState<WooCommerceStatus | null>(null)
  ...
  const load = useCallback(async () => {
    if (!botId) return
    const [result, meta, shopify, woo] = await Promise.all([fetch(`/api/integrations?botId=${botId}`).then(response => response.json()), fetch(`/api/meta/status?botId=${botId}`).then(response => response.json()), fetch(`/api/shopify/status?botId=${botId}`).then(response => response.json()), fetch(`/api/woocommerce/status?botId=${botId}`).then(response => response.json())])
    setItems(result.data || []); setMetaStatus(meta.success ? meta.data : null); setShopifyStatus(shopify.success ? shopify.data : null); setWooStatus(woo.success ? woo.data : null)
  }, [botId])
  useEffect(() => { load() }, [load])
  useEffect(() => {
    ...
    const woo = params.get('woo')
    if (meta === 'instagram-connected') setMessage('Instagram collegato correttamente. I nuovi Direct possono essere gestiti dal chatbot.')
    if (shopify === 'connected') setMessage('Shopify collegato e catalogo sincronizzato correttamente.')
    if (shopify === 'error') setMessage(params.get('detail') || 'Collegamento Shopify non riuscito.')
    if (woo === 'connecting') setMessage('WooCommerce autorizzato. La sincronizzazione del catalogo e dei webhook è in corso.')
    if (woo === 'error') setMessage(params.get('detail') || 'Collegamento WooCommerce non riuscito.')
  }, [])
  ...
  return <div className="mx-auto max-w-[1500px] p-4 lg:p-7">
    ...
    {editing && <IntegrationDialog editing={editing} botId={botId} config={config} setConfig={setConfig} busy={busy} message={message} metaStatus={metaStatus} shopifyStatus={shopifyStatus} wooStatus={wooStatus} onClose={() => setEditing(null)} onSave={save} onTest={() => test(editing)} onConnectMeta={connectMeta} onConnectShopify={connectShopify} onConnectWooCommerce={connectWooCommerce} />}
  </div>
}

function IntegrationDialog({ editing, botId, config, setConfig, busy, message, metaStatus, shopifyStatus, wooStatus, onClose, onSave, onTest, onConnectMeta, onConnectShopify, onConnectWooCommerce }: { editing: Integration; botId: string; config: Record<string, string>; setConfig: (value: Record<string, string>) => void; busy: boolean; message: string; metaStatus: MetaStatus | null; shopifyStatus: ShopifyStatus | null; wooStatus: WooCommerceStatus | null; onClose: () => void; onSave: () => void; onTest: () => void; onConnectMeta: (provider: 'whatsapp' | 'instagram') => void; onConnectShopify: () => void; onConnectWooCommerce: () => void }) {
  const meta = isMeta(editing.provider) ? metaStatus?.[editing.provider] : undefined
  const shopifyNeedsOrderAuthorization = Boolean(shopifyStatus?.connected && !shopifyStatus.orderTracking?.granted)
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-950/45 p-4 backdrop-blur-sm"><div role="dialog" aria-modal="true" aria-labelledby="integration-title" className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-2xl bg-white p-6 shadow-hard"><div className="flex items-start gap-3"><div className="flex h-11 w-11 items-center justify-center rounded-xl text-xs font-bold text-white" style={{ backgroundColor: editing.color }}>{editing.initials}</div><div><p className="eyebrow">Configurazione</p><h2 id="integration-title" className="text-lg font-bold">{editing.name}</h2></div></div><div className="mt-5 space-y-4">
    ...
    {isMeta(editing.provider) ? <MetaConnectionPanel botId={botId} provider={editing.provider} state={meta} webhookUrl={metaStatus?.webhookUrl} /> : editing.provider === 'shopify' ? <ShopifyConnectionPanel state={shopifyStatus} /> : editing.provider === 'woocommerce' ? <WooCommerceConnectionPanel state={wooStatus} /> : editing.mode === 'native' && <div className="rounded-xl bg-emerald-50 p-4 text-xs leading-5 text-emerald-700"><Check className="mb-2 h-4 w-4" />Questa funzione è già inclusa nella piattaforma. Attivandola apparirà tra i canali collegati.</div>}
  ...
  </div><div className="mt-6 flex justify-end gap-2"><Button variant="secondary" onClick={onClose}>Annulla</Button>{editing.connection && editing.mode === 'configuration' && !['shopify', 'woocommerce'].includes(editing.provider) && <Button variant="secondary" onClick={onTest} disabled={busy} icon={<Wifi className="h-4 w-4" />}>Testa</Button>}{isMeta(editing.provider) ? <Button onClick={() => onConnectMeta(editing.provider as 'whatsapp' | 'instagram')} disabled={busy || meta?.connected || !meta?.configured} icon={busy ? <Loader2 className="h-4 w-4 animate-spin" /> : <ShieldCheck className="h-4 w-4" />}>{meta?.connected ? 'Collegato' : 'Continua con Meta'}</Button> : editing.provider === 'shopify' ? <Button onClick={onConnectShopify} disabled={busy || Boolean(shopifyStatus?.connected && !shopifyNeedsOrderAuthorization) || !shopifyStatus?.configured} icon={<ShieldCheck className="h-4 w-4" />}>{shopifyNeedsOrderAuthorization ? 'Autorizza tracking ordini' : shopifyStatus?.connected ? 'Shopify collegato' : 'Continua con Shopify'}</Button> : editing.provider === 'woocommerce' ? <Button onClick={onConnectWooCommerce} disabled={busy || wooStatus?.connected || !wooStatus?.configured} icon={<ShieldCheck className="h-4 w-4" />}>{wooStatus?.connected ? 'WooCommerce collegato' : 'Continua con WooCommerce'}</Button> : <Button onClick={onSave} disabled={busy} icon={busy ? <Loader2 className="h-4 w-4 animate-spin" /> : <Check className="h-4 w-4" />}>Salva</Button>}</div>{editing.provider === 'public-page' && botId && <a className="mt-4 flex items-center justify-center gap-1 text-xs font-semibold text-brand-700" href={`/agent/${botId}`} target="_blank" rel="noopener noreferrer">Apri pagina pubblica <ExternalLink className="h-3 w-3" /></a>}</div></div>
}

function ShopifyConnectionPanel({ state }: { state: ShopifyStatus | null }) {
  return <div className={`rounded-xl border p-4 ${state?.configured ? 'border-emerald-200 bg-emerald-50' : 'border-amber-200 bg-amber-50'}`}>
    <div className="flex items-start gap-3"><ShieldCheck className={`mt-0.5 h-5 w-5 shrink-0 ${state?.configured ? 'text-emerald-600' : 'text-amber-600'}`} /><div className="min-w-0"><p className="text-xs font-semibold text-gray-900">Installazione ufficiale Shopify</p><p className="mt-1 text-[11px] leading-5 text-gray-600">{state?.connected ? `${state.shopDomain} è collegato. Catalogo e aggiornamenti prodotto sono sincronizzati automaticamente.` : state?.configured ? 'Si aprirà Shopify: il cliente autorizza LitX senza comunicarti password o token. I token offline vengono cifrati e rinnovati automaticamente.' : 'Configura una sola volta Client ID, Client Secret e callback nella tua Shopify App. I clienti dovranno soltanto premere Continua con Shopify.'}</p>{state?.connected && <div className={`mt-3 rounded-lg border px-3 py-2 text-[10px] ${state.orderTracking?.granted && state.orderTracking.pcdStatus !== 'required' ? 'border-emerald-200 bg-white/80 text-emerald-700' : 'border-amber-200 bg-amber-50 text-amber-800'}`}><strong className="block">Tracking ordini</strong>{!state.orderTracking?.granted ? 'Ricollega il negozio per autorizzare read_orders.' : state.orderTracking.pcdStatus === 'required' ? 'Abilita Protected Customer Data livello 2 e il campo Email nel Shopify Dev Dashboard.' : 'Permesso read_orders concesso. La verifica PCD avverrà al primo controllo reale.'}</div>}{state?.themeEditorUrl && <a href={state.themeEditorUrl} target="_blank" rel="noopener noreferrer" className="mt-3 inline-flex items-center gap-1.5 rounded-lg bg-brand-600 px-3 py-2 text-[10px] font-semibold text-white hover:bg-brand-700">Attiva widget nel tema <ExternalLink className="h-3 w-3" /></a>}{state?.callbackUrl && <p className="mt-3 break-all rounded-lg bg-white/80 p-2 font-mono text-[9px] text-gray-500">Callback: {state.callbackUrl}</p>}{state?.webhookUrl && <p className="mt-2 break-all rounded-lg bg-white/80 p-2 font-mono text-[9px] text-gray-500">Webhook: {state.webhookUrl}</p>}{state?.lastError && <p className="mt-2 text-[10px] text-red-700">{state.lastError}</p>}</div></div>
  </div>
}

function WooCommerceConnectionPanel({ state }: { state: WooCommerceStatus | null }) {
  return <div className={`rounded-xl border p-4 ${state?.configured ? 'border-purple-200 bg-purple-50' : 'border-amber-200 bg-amber-50'}`}>
    <div className="flex items-start gap-3"><ShieldCheck className={`mt-0.5 h-5 w-5 shrink-0 ${state?.configured ? 'text-purple-700' : 'text-amber-600'}`} /><div className="min-w-0"><p className="text-xs font-semibold text-gray-900">Autorizzazione ufficiale WooCommerce</p><p className="mt-1 text-[11px] leading-5 text-gray-600">{state?.connected ? `${state.storeOrigin} è collegato. Prodotti e ordini aggiornano automaticamente LitX.` : state?.configured ? 'Si aprirà WordPress/WooCommerce: l’amministratore approva LitX e le chiavi vengono inviate direttamente al server, cifrate e mai mostrate nel browser.' : 'Serve un dominio pubblico HTTPS per ricevere in modo sicuro chiavi e webhook WooCommerce.'}</p>{state?.callbackUrl && <p className="mt-3 break-all rounded-lg bg-white/80 p-2 font-mono text-[9px] text-gray-500">Callback: {state.callbackUrl}</p>}{state?.webhookUrl && <p className="mt-2 break-all rounded-lg bg-white/80 p-2 font-mono text-[9px] text-gray-500">Webhook: {state.webhookUrl}</p>}{state?.lastError && <p className="mt-2 text-[10px] text-red-700">{state.lastError}</p>}</div></div>
  </div>
}
