...
export function IntegrationMarketplace({ initialCategory = 'all' }: { initialCategory?: string }) {
  ...
  const load = useCallback(async () => {
    if (!botId) return
    const [result, meta, shopify] = await Promise.all([fetch(`/api/integrations?botId=${botId}`).then(response => response.json()), fetch(`/api/meta/status?botId=${botId}`).then(response => response.json()), fetch(`/api/shopify/status?botId=${botId}`).then(response => response.json())])
    setItems(result.data || []); setMetaStatus(meta.success ? meta.data : null); setShopifyStatus(shopify.success ? shopify.data : null)
  }, [botId])
  useEffect(() => { load() }, [load])
  useEffect(() => {
  ...
  return <div className="mx-auto max-w-[1500px] p-4 lg:p-7">
    ...
    {editing && <IntegrationDialog editing={editing} botId={botId} config={config} setConfig={setConfig} busy={busy} message={message} metaStatus={metaStatus} shopifyStatus={shopifyStatus} onClose={() => setEditing(null)} onSave={save} onTest={() => test(editing)} onConnectMeta={connectMeta} onConnectShopify={connectShopify} />}
  </div>
}

function IntegrationDialog({ editing, botId, config, setConfig, busy, message, metaStatus, shopifyStatus, onClose, onSave, onTest, onConnectMeta, onConnectShopify }: { editing: Integration; botId: string; config: Record<string, string>; setConfig: (value: Record<string, string>) => void; busy: boolean; message: string; metaStatus: MetaStatus | null; shopifyStatus: ShopifyStatus | null; onClose: () => void; onSave: () => void; onTest: () => void; onConnectMeta: (provider: 'whatsapp' | 'instagram') => void; onConnectShopify: () => void }) {
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-950/45 p-4 backdrop-blur-sm"><div role="dialog" aria-modal="true" aria-labelledby="integration-title" className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-2xl bg-white p-6 shadow-hard"><div className="flex items-start gap-3"><div className="flex h-11 w-11 items-center justify-center rounded-xl text-xs font-bold text-white" style={{ backgroundColor: editing.color }}>{editing.initials}</div><div><p className="eyebrow">Configurazione</p><h2 id="integration-title" className="text-lg font-bold">{editing.name}</h2></div></div><div className="mt-5 space-y-4">
    ...
    {isMeta(editing.provider) ? <MetaConnectionPanel botId={botId} provider={editing.provider} state={meta} webhookUrl={metaStatus?.webhookUrl} /> : editing.provider === 'shopify' ? <ShopifyConnectionPanel state={shopifyStatus} /> : editing.mode === 'native' && <div className="rounded-xl bg-emerald-50 p-4 text-xs leading-5 text-emerald-700"><Check className="mb-2 h-4 w-4" />Questa funzione è già inclusa nella piattaforma. Attivandola apparirà tra i canali collegati.</div>}
