...
function isTenantReadyApi(request: NextRequest) {
  ...
  if (path === '/api/dashboard/traces' || path === '/api/shopify/status' || path === '/api/woocommerce/status') return request.method === 'GET'
  if (path === '/api/prompt-improve' || path === '/api/widgets/generate') return request.method === 'POST'
  if (path === '/api/backstage/chat') return request.method === 'POST'
  if (path === '/api/backstage/sessions') return request.method === 'GET' || request.method === 'POST'
  if (path === '/api/privacy/retention') return request.method === 'GET' || request.method === 'POST'
  if (path === '/api/privacy/visitor-data') return request.method === 'GET' || request.method === 'DELETE'
  if (path === '/api/helpdesk/views') return request.method === 'GET' || request.method === 'POST'
  if (path === '/api/shopify/oauth/start' || path === '/api/woocommerce/oauth/start') return request.method === 'GET'
  ...
  if (/^\/api\/messages\/[0-9a-f-]{36}\/feedback$/i.test(path)) return request.method === 'POST'
  if (/^\/api\/decisions\/[0-9a-f-]{36}\/trace$/i.test(path)) return request.method === 'GET'
  if (/^\/api\/response-revisions\/[0-9a-f-]{36}$/i.test(path)) return request.method === 'PATCH'
  if (/^\/api\/response-revisions\/[0-9a-f-]{36}\/(archive|publish)$/i.test(path)) return request.method === 'POST'
  if (/^\/api\/backstage\/sessions\/[0-9a-f-]{36}$/i.test(path)) return request.method === 'GET'
  if (/^\/api\/backstage\/drafts\/[0-9a-f-]{36}$/i.test(path)) return request.method === 'PATCH'
  if (/^\/api\/backstage\/drafts\/[0-9a-f-]{36}\/(apply|reject|rollback|simulate)$/i.test(path)) return request.method === 'POST'
  if (/^\/api\/helpdesk\/views\/[0-9a-f-]{36}$/i.test(path)) return request.method === 'PATCH' || request.method === 'DELETE'
  ...
  if (/^\/api\/workspaces\/[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}\/members$/i.test(path)) {
    return request.method === 'GET'
  }
  if (/^\/api\/workspaces\/[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}\/audit$/i.test(path)) {
    return request.method === 'GET'
  }
  if (/^\/api\/workspaces\/[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}\/members\/[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(path)) {
    return request.method === 'PATCH' || request.method === 'DELETE'
  }
  if (/^\/api\/workspaces\/[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}\/invitations\/[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(path)) {
